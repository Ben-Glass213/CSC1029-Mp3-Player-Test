...
public class SystemApp {
	...
	public static void main(String[] args) {
		...
		/**
		 * Creating an array which will act as our menu options which will then be used
		 * to print the menu on screen. this method will also check to see if user input
		 * == quit or not.
		 */
	...
	public static void userOption(int userChoice) {
		/**
		 * userOption uses switches to process user inputs, ie if the user picks 1, the
		 * switch will use case 1 and call the selectFromFullList method.
		 */
		switch (userChoice) {
		case 1:
			selectFromFullList();
		...
		case 4:
			 ...
			 * User will input a title, artist and duration along with genre and will throw
			 * an exception if it doesn't get the correct info ie == null. User info will
			 * then be stored and passed to the addNewTune method to add it to the list of
			 * tunes
			 */
		...
		case 7:
			...
			break;
		case 8:
			// will print a new line and method above (userInput == quit) then prints
			// GoodBye!
			System.out.println();
			break;
		}
	...
	/**
	 * This method is called when the user selects to add a new song to the mp3
	 * player
	 * 
	...
	/**
	 * Allows the user to select and see the songs from the list allowing them to
	 * play them
	 */
	public static void selectFromFullList() {
		// validation to see if there is any info to be displayed from the getTuneInfo
		// method
		if (myPlayer.getTuneInfo() != null) {
			...
			/*
			 * validation to check to see if user input is picking a song that is in the
			 * list, is user is not picking a number that is out of bounds. also plays the
			 * song that the user selects.
			 */
	...
	public static void selectTuneByArtist() {
		...
		/**
		 * Creating a new menu and showing the list of artist to choose from, where the
		 * user will then select and artist.
		 */
		...
		if (info.length != 0) {
			...
			/*
			 * creating a new menu to which is then used once the user has selected an
			 * artist. songs from the artist are then displayed then the user is asked to
			 * select a song they want to play.
			 */
	...
	public static void selectTuneByGenre() {
		...
		/**
		 * switch used to allow the user to pick an option based on the order from the
		 * array above
		 */
		switch (choice) {
		case 1:
		...
		/**
		 * similar method to the get artist method. This is used to collect and display
		 * information about songs based on the genre that the user picks.
		 * 
		 * Creating a new menu and showing the list of genres to choose from, where the
		 * user will then select a genre of music.
		 */
		...
		if (info.length != 0) {
			 ...
			 * songs from the select genre are then displayed, then the user is asked to
			 * select a song they want to play.
			 */
	...
	/**
	 * method used to display the top ten songs that are played from the list. Also
	 * includes tunes that have been added.
	 */
